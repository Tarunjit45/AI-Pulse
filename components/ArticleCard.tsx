...
const ScoreBar = ({ label, score, colorClass }: { label: string; score: number; colorClass: string }) => (
  <div className="mb-4 md:mb-5 group">
    <div className="flex justify-between text-[10px] md:text-xs font-bold uppercase tracking-widest text-gray-400 mb-1.5 md:mb-2 group-hover:text-white transition-colors">
      <span>{label}</span>
    ...
    return (
        <div className="flex flex-col h-full rounded-2xl border border-white/5 bg-black/20 overflow-hidden text-sm">
            <div className="flex-grow overflow-y-auto p-3 md:p-4 space-y-3 custom-scrollbar" ref={scrollRef}>
                {messages.map((m, i) => (
                    <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] rounded-2xl px-3 py-2 md:px-4 md:py-3 text-xs md:text-sm leading-relaxed shadow-lg ${
                            m.role === 'user' 
            ...
            <div className="p-2 md:p-3 bg-black/30 border-t border-white/5 flex gap-2">
                <input 
                    className="flex-grow bg-gray-900/50 border border-gray-700/50 rounded-full px-4 py-2 text-xs md:text-sm text-white focus:outline-none focus:border-neon-cyan focus:ring-1 focus:ring-neon-cyan/50 transition-all placeholder:text-gray-600 font-medium"
                ...
                <button 
                    onClick={handleSend}
                    className="p-2 md:p-3 bg-neon-cyan/10 hover:bg-neon-cyan/20 text-neon-cyan border border-neon-cyan/30 rounded-full transition-colors active:scale-95"
                >
...
const renderMarkdown = (text: string) => {
  ...
  let html = text
    ...
    .replace(/\n\n/g, '</p><p class="mb-3 md:mb-4 leading-relaxed text-gray-300">')
    .replace(/\n/g, '<br />');

  return `<p class="mb-3 md:mb-4 leading-relaxed text-gray-300 font-body text-sm md:text-[15px]">${html}</p>`;
};
...
export const ArticleCard: React.FC<ArticleCardProps> = ({ article, isGenerating, onGenerate, onSwipe, isTopCard }) => {
  ...
  return (
    ...
    >
      ...
      {/* Hero Image Section */}
      <div className="relative h-48 md:h-60 shrink-0 group overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0f1e] via-[#0a0f1e]/40 to-transparent z-10" />
        
        {/* Category Badge */}
        <div className="absolute top-3 left-3 md:top-4 md:left-4 z-20">
            <span className="px-2 py-1 md:px-3 md:py-1.5 bg-black/60 backdrop-blur-md border border-white/10 rounded-lg text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-white shadow-lg">
        ...
        <button 
            ...
            className="absolute top-3 right-3 md:top-4 md:right-4 z-20 p-2 md:p-2.5 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full text-white transition-all border border-white/10 no-drag hover:scale-105 active:scale-95 touch-manipulation"
        >
            {isSpeaking ? (
        ...
        <div className="absolute bottom-0 left-0 p-4 md:p-6 z-20 w-full">
            <h2 className="text-xl md:text-2xl font-display font-bold text-white leading-tight drop-shadow-xl line-clamp-2">{article.title}</h2>
        </div>
      </div>
      ...
      <div className="flex mx-3 md:mx-4 mt-2 p-1 bg-white/5 rounded-xl backdrop-blur-md border border-white/5 no-drag relative z-20 shrink-0">
        {[
            ...
            <button
                ...
                className={`flex-1 py-1.5 md:py-2 text-[10px] md:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 relative ${activeTab === tab.id ? 'text-black shadow-lg' : 'text-gray-400 hover:text-white'}`}
            >
                {activeTab === tab.id && (
      ...
      {/* Content Area */}
      <div className="p-4 md:p-6 flex-grow overflow-y-auto no-drag custom-scrollbar relative">
        
        {activeTab === 'story' && (
            <div className="animate-fadeIn">
                <div 
                  className="prose prose-invert prose-p:text-gray-300 max-w-none mb-4 md:mb-6"
                  dangerouslySetInnerHTML={{ __html: articleHtml }}
                />
                
                <div className="flex items-center gap-2 mt-2 md:mt-4 opacity-60 hover:opacity-100 transition-opacity">
                    <div className="h-px bg-gradient-to-r from-transparent via-gray-500 to-transparent flex-grow"></div>
                    <a href={article.sources[0]} target="_blank" rel="noopener noreferrer" className="text-[9px] md:text-[10px] text-neon-cyan font-mono uppercase tracking-widest hover:underline truncate max-w-[150px] md:max-w-[200px]">
                        SOURCE UPLINK
                    </a>
                    <div className="h-px bg-gradient-to-r from-transparent via-gray-500 to-transparent flex-grow"></div>
                </div>
        ...
        {activeTab === 'analysis' && article.analysis && (
            <div className="space-y-5 md:space-y-7 animate-fadeIn h-full flex flex-col justify-center">
                ...
                <div className="relative p-4 md:p-5 bg-gradient-to-br from-neon-purple/10 to-transparent rounded-xl md:rounded-2xl border border-neon-purple/20">
                    <div className="absolute -top-2.5 left-4 bg-[#0a0f1e] px-2 text-[10px] md:text-xs font-bold text-neon-purple uppercase tracking-widest">Projection</div>
                    <p className="text-gray-200 italic font-body text-xs md:text-sm leading-relaxed">
                        "{article.analysis.prediction}"
                    </p>
                </div>

                <div className="relative p-4 md:p-5 bg-gradient-to-br from-blue-900/10 to-transparent rounded-xl md:rounded-2xl border border-blue-500/20">
                    <div className="absolute -top-2.5 left-4 bg-[#0a0f1e] px-2 text-[10px] md:text-xs font-bold text-blue-400 uppercase tracking-widest">Term Decoded</div>
                    <div className="flex flex-col gap-1">
                        <span className="text-white font-display font-bold text-base md:text-lg">{article.analysis.technicalTerm}</span>
                        <span className="text-gray-400 text-xs md:text-sm leading-relaxed">{article.analysis.simpleDefinition}</span>
                    </div>
                </div>
            </div>
        )}
      ...
      {/* Footer Actions */}
      <div 
        className="p-3 md:p-5 bg-[#0a0f1e]/90 backdrop-blur-xl border-t border-white/5 flex gap-2 md:gap-3 no-drag z-30 shrink-0"
      >
        <button
            onClick={() => onGenerate(article.body, 'LinkedIn')}
            disabled={isGenerating}
            className="flex-1 py-2.5 md:py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs md:text-sm font-bold shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:shadow-[0_0_30px_rgba(37,99,235,0.5)] transition-all active:scale-95 disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-1.5 md:gap-2 group touch-manipulation"
        >
            <LinkedInIcon className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover:scale-110 transition-transform" />
            <span>Post</span>
        </button>
        <button
            onClick={() => onGenerate(article.body, 'Medium')}
            disabled={isGenerating}
            className="flex-1 py-2.5 md:py-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-white text-xs md:text-sm font-bold shadow-lg border border-white/5 transition-all active:scale-95 disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-1.5 md:gap-2 group touch-manipulation"
        >
            <MediumIcon className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover:scale-110 transition-transform" />
            <span>Blog</span>
        </button>
        <button
            onClick={() => {
            ...
            className="p-2.5 md:p-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 border border-white/5 transition-all active:scale-95 hover:text-neon-cyan touch-manipulation"
        >
